...
public class Sort {
    ...
    public static void checkBalance(Scanner sc, List<Bank> KB) {
        ...
        for (int i = 0; i < KB.size(); i++) {
            if (KB.get(i).Account.equals(account)) {
                System.out.println("비밀번호를 입력해주세요");
                String password = sc.nextLine();
                if (KB.get(i).password.equals(password)) {
                    System.out.println(KB.get(i).balance);
                }
        ...
        for (int i = 0; i < KB.size(); i++) {
            if (KB.get(i).Account.equals(account)) {
                ...
                if (KB.get(i).password.equals(password)) {
                    ...
                    KB.get(i).balance += add;
                    System.out.println("충전후 잔액은 " + KB.get(i).balance + "원 입니다");

                }
            }
        }
    }
    public static void validate(Scanner sc, List<Bank> KB){
        System.out.println("계좌번호를 입력해주세요.");
        String account = sc.nextLine();
        for (Bank.bank.KB) {
